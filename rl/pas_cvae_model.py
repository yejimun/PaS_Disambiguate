...
class Label_VAE(nn.Module):
    def __init__(self, args):
        ...
        self.decoder = nn.Sequential(
            init_(nn.Linear(args.rnn_output_size*2, 32*3*3)),               
            View((-1, 32, 3, 3)),              
            nn.ReLU(), 
            nn.BatchNorm2d(32),
        ...
        vector_length = 4
        self.encoder_embed_layer = nn.Linear(vector_length, args.rnn_output_size)
        self.decoder_embed_layer = nn.Linear(vector_length, args.rnn_output_size)

        self.linear_mu = nn.Linear(args.rnn_output_size*3, args.rnn_output_size)
        self.linear_var = nn.Linear(args.rnn_output_size*3, args.rnn_output_size)
    ...
    def encode(self, grid, vector): 
        c = self.encoder_embed_layer(vector)       
    ...
    def decode(self, z, vector):
        c = self.decoder_embed_layer(vector)
        z = torch.cat((z, c), -1)
    ...
    def forward(self, grid, vector):
        ...
        """
        z_mu, z_log_variance, z = self.encode(grid, vector) # z:(1, 1, args.rnn_output_size)
        
        decoded = self.decode(z, vector) 
...
class Sensor_VAE(nn.Module):
    ...
    def forward(self, grid, vector):
        """
        Args:
            grid:(T*N, 4, *grid_shape)
        Return:
            z : (T*N, 1, args.rnn_output_size)
            decoded : (T*N, 1, *grid_shape)
        """
        z_mu, z_log_variance, z = self.encode(grid, vector)     
