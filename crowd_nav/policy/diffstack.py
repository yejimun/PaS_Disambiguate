...
class PaS_DiffStack(Policy):
	...
	def clip_action(self, raw_action, v_pref, prev_v, time_step, a_pref=1.):
		...
		# else:
		# 	raw_action[0] = np.clip(np.abs(raw_action[0]), 0., v_pref) # action[0] is change of v
		# 	raw_action[1] = np.clip(raw_action[1], -np.pi/3., np.pi/3.) # action[1] is change of theta
