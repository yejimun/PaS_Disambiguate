...
class MPPI_Planner():
    ...
    def dynamics(self, state, action):
        ...
        if self.kinematics == "holonomic":
            ...
            next_r_state = torch.stack([next_x, next_y, next_theta, action[...,0]], dim=-1)
        else:
            raise ValueError("Invalid kinematics")        
        return next_r_state
    ...
    def visualize_disambig_traj(self,r_state, next_r_state, goal_pos, highlight_indices=None, map_xy=None, sensor_grid=None, wall_polygons=None, h_states=None, lowest_interest_indices=None, lowest_cost_ind=None, cost=None):
        ...
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        # count how many is already saved
        k = len(glob.glob(save_dir+'/vis_traj*.png')) 
        ...
        if highlight_indices is not None:
            # for idx in  highlight_indices:
            # ax.plot(r_state[0,highlight_indices,0].cpu(), r_state[0,highlight_indices,1].cpu(), 'b+', markersize=10)
            # ax.plot(next_r_state[0,highlight_indices,0].cpu(), next_r_state[0,highlight_indices,1].cpu(), 'bo', markersize=10)
            ax.plot(r_state[0,highlight_indices,0].cpu(), r_state[0,highlight_indices,1].cpu(), 'bo', markersize=10)
        ...
        # ax.legend(["Next state", "Goal",  "Disambig next state", "Lowest DisambigC current state", "Lowest cost next state"]) 
    ...
    def visualize(self,vis_grids, goal_pos, disambig_c):
        ...
        if not os.path.exists(save_dir):
            os.makedirs(save_dir)
        # count how many is already saved
        k = len(glob.glob(save_dir+'/vis_plot*.png')) 
    ...
    def running_cost(self, state, ob, next_state, decoded, wall_polygons, t):
        ...
        if 'pas' in self.config.robot.policy and torch.any(decoded_in_unknown):            
            ...
            PaS_occupied_indices = torch.where(decoded_in_unknown_flat>0.4)
            if len(PaS_occupied_indices[0])>0:
                ...
                PaS_occupied_prob = decoded_in_unknown_flat[PaS_occupied_indices] # (N_indices)
